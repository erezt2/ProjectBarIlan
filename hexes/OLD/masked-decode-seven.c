...
uint8_t g[SIZE];
uint8_t           error[SIZE], s[21];
int             seed;
//uint8_t             numerr, /*errpos[SIZE],*/ decerror = 0;
uint8_t si = 0;
uint8_t t2;
uint8_t mem[6][22] = {0};
uint8_t store[7][131] = {0};
...
int8_t 
syndrome(uint8_t idx, uint8_t* pt) // why is it only 2*t long?
/*
...
handle(uint8_t* pt, uint8_t* buffer)
{
 ...
 //        recd[i] ^= info[i];
    ...
    for(i=0;i<6;i++){ // TODO: maybe change temp to a normal pointer?
        for(j=0;j<length;j++){
            store[i+1][j] ^= store[0][j];
        }
        syndrome(i+1, pt);
        for (j=0;j<=t2;j++) {
            s[j] ^= mem[i][j];
...
// }
uint8_t key_index = 0;
uint8_t get_key(uint8_t* k, uint8_t len) // TODO: verify once again the syndrome from python is equivalent
{
    if(key_index == 6) {
        key_index = 0;
    }
    ...
    for(uint8_t i=0;i<=t2;i++){
        mem[key_index][i+1] = k[i];
    }
	simpleserial_put('r', 20, (&mem[key_index][0])+1);
    key_index++;
	return 0x00;
}
uint8_t restart_key(uint8_t* k, uint8_t len) // TODO: verify once again the syndrome from python is equivalent
{
    key_index = 0;
    si = 0;
	simpleserial_put('r', 1, &si);
	return 0x00;
}

uint8_t get_si(uint8_t* k, uint8_t len) // TODO: verify once again the syndrome from python is equivalent
{
	simpleserial_put('r', 1, &si);
	return 0x00;
}

// uint8_t get_syndrome(uint8_t* pt, uint8_t len) {
//     for (uint8_t i = 0; i <length; i++){ // puts input in recd (to decode later)
// 		recd[i] = ((pt[i/8] >> (i%8)) & 1);
//     }
//     syndrome(recd);
//     simpleserial_put('r', 20, s+1);
//     return 0;
// }
...
int main(void)
{
    ...
    // simpleserial_addcmd('a', 16,  get_syndrome);
    simpleserial_addcmd('r', 0, restart_key);
    simpleserial_addcmd('g', 0, get_si);
